...
product_url() - 155,brown-rice ==> globemart.com/product/155-brown-rice
...
$URL = "https://localhost:8888/globemart/";
$URL13 = "https://globemart.com/globemart2013/";
$URL13 = $URL;
...
$headers .= 'From: Globemart <info@globemart.com>' . "\r\n";
...
// increase rank points for activity, used in search sorting
function rankup($comp_id, $rank){
	mysql_query("UPDATE companies SET rank_points=rank_points+$rank WHERE id='$comp_id'");
}

// rank points for generated tradedocs (3 points if reached 3 TD, 5 points for 10 TD, 10 points for 20+ TD; deleted TD don't count)
function rank_tradedocs($comp_id){
	global $R_TRADEDOCS03, $R_TRADEDOCS10, $R_TRADEDOCS20;
	
	mysql_query("UPDATE companies SET tradedocs_generated=tradedocs_generated+1 WHERE id='".$comp_id."'");
	
	$sql = "SELECT tradedocs_generated, tradedocs_reached FROM companies WHERE id='".$comp_id."'";
	$result  = mysql_query($sql);
	$row     = mysql_fetch_array($result, MYSQL_ASSOC);
	$tradedocs_generated = $row["tradedocs_generated"];	// current number of generated tradedocs
	$tradedocs_reached   = $row["tradedocs_reached"];	// can be 0,3,10,20 - flags that changes only once, increasingly
	
	if ($tradedocs_generated==3 and $tradedocs_reached==0){
		mysql_query("UPDATE companies SET tradedocs_reached='$tradedocs_generated' WHERE id='$comp_id'");
		rankup($comp_id, $R_TRADEDOCS03);
	}
	elseif ($tradedocs_generated==10 and $tradedocs_reached==3){
		mysql_query("UPDATE companies SET tradedocs_reached='$tradedocs_generated' WHERE id='$comp_id'");
		rankup($comp_id, $R_TRADEDOCS10);
	}
	elseif ($tradedocs_generated==20 and $tradedocs_reached==10){
		mysql_query("UPDATE companies SET tradedocs_reached='$tradedocs_generated' WHERE id='$comp_id'");
		rankup($comp_id, $R_TRADEDOCS20);
	}
}
...
// update members for IP
function update_db_ip($memb_id){
	if (is_numeric($memb_id)){
		$ip = get_ip();
		
		$obj = new my_geo_class();
		$rec = $obj->get_data($ip);
		...
		if (is_numeric($latitude) and is_numeric($longitude) and $latitude+$longitude!=0 and $ip!=""){
			$sql = "UPDATE members SET ip='$ip', ip_lat='$latitude', ip_long='$longitude', 
										ip_country='$country_name', ip_city='$city', ip_state='$region' 
					WHERE id='$memb_id'";
			mysql_query($sql);
		}
		elseif ($ip!=""){
			$sql = "UPDATE members SET ip='$ip' WHERE id='$memb_id'";
			mysql_query($sql);
		}
	}
}

// register visits of smb's pages for statistics (unique visitors this month, world map of visitors, etc)
function reg_visit($comp_id){
	global $_COOKIE_comp_id, $_COOKIE_memb_id;
	$cookie_comp_id = isset($_COOKIE_comp_id) ? $_COOKIE_comp_id : -1;
	if ($comp_id!=$cookie_comp_id and $comp_id!=0){		// exclude yourself
		$ip = get_ip();
		$today = date("Y-m-d");
		
		$result = mysql_query("SELECT COUNT(1) as cn FROM visits WHERE comp_id='$comp_id' AND watched_by_ip='$ip' AND date_watched='$today'");
		$row = mysql_fetch_array($result);
		if ($row["cn"]==0){
			mysql_query("INSERT INTO visits SET comp_id='$comp_id', watched_by_ip='$ip', date_watched='$today'");
		}
	}
}

function cast_price($price, $currency, $price_max=""){
	global $ar_curr2;
	if ($price_max==""){
		if ($price=="" or $price==0) 
			return "";
			...
			$price2 = $ar_curr2[$currency].number_format($price,2);
			if ($currency!="USD") $price2.= " ".$currency;
			return $price2;
		}
	}
	else{
		if ($currency=="") $currency = "USD";
		$cur_sign = $ar_curr2[$currency];
		if ($currency=="USD") $currency = "";
		
		if ($price==0 and $price_max==0)
			$fob_str = "";
		elseif ($price==0 and $price_max!=0)
			$fob_str = "up to ".$cur_sign.number_format($price_max,2)." ".$currency;
		elseif ($price!=0 and $price_max==0)
			$fob_str = "from ".$cur_sign.number_format($price,2)." ".$currency;
		else
			$fob_str = "from ".$cur_sign.number_format($price,2)." to ".$cur_sign.number_format($price_max,2)." ".$currency;
...
function sender_joinfor($toemail, $custom_subject, $custom_message, $firstname, $comp_url, $login, $pwd, $arr_sections="", $memb_id){
	...
	if (is_array($arr_sections)){
		...
		$section4 = "You can change your password in your Profile once you have activated your account.
					...
					Globemart Team
					<br/><br/>
					<a href='https://globemart.com/about' style='color:#1686A2;'>Learn about Globemart</a>";
		$section5 = 1;		
		$section6 = "Have any questions? Please contact us at <span style='color:#1686A2;'>info@globemart.com</span>";
	}
	...
	if ($section5!=""){
		$mail_body.= "<tr align='center'>
			<td colspan=2 style='font-size:20px' height=120>
				<img src='https://globemart.com/Media/home_ads/1768660578_ew15_faviconlogo.png' border=0 width=100 alt='ExpoWest' title='ExpoWest' align='absmiddle'>
	...
	$mail_body.= "<tr align='center' valign='middle' style='background-color:#47C1BE;' height=120>
		<td colspan=2>
			<a href='https://globemart.com/terms' style='color:#FFF'>Terms</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href='https://globemart.com/privacy' style='color:#FFF'>Privacy</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a href='https://globemart.com/notifications_settings.php' style='color:#FFF'>Unsubscribe</a>
		</td>
	</tr>";
...
// 155,brown-rice ==> globemart.com/product/155-brown-rice ==> globemart.com/product/217854836-brown-rice
